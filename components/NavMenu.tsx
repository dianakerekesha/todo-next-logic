...
function AuthButton() {
  ...
  if (session) {
    return (
      <div className="mb-4">
        <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">{session?.user?.name}</p>
      ...
      <ul className="space-y-2">
        ...
        {session && (
          <>
            ...
            <li>
              <Link href="/serverAction" className={getLinkStyle("/serverAction")}>
                Server Action
              </Link>
            </li>

            <li>
              <Link href="/apiFromClient" className={getLinkStyle("/apiFromClient")}>
                ApiFromClient
              </Link>
            </li>

            <li>
              <Link href="/apiFromServer" className={getLinkStyle("/apiFromServer")}>
                ApiFromServer
              </Link>
            </li>
          </>
